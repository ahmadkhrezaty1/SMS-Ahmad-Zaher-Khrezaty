...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, default_language')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	...
	public function uploadImage($field, $id)
    {
    	...
    	mkdir('uploads/faces/'.$id);
        $config['upload_path'] = 'uploads/faces/'.$id.'/';
	...
	public function set_session($user)
	{
		...
		$session_data = [
		    ...
		    'site_lang'			   => $user->default_language,
		    'last_check'           => time(),
		];
